...
def me_update():
    ...
    if request.method == "GET" and session.get('id') is not None:               
        ...
        save_title=request.form.get('notitle').encode('utf-8')
        ...
        try:        
           ...
           if allowed_file(file.filename) is False:
               res_filename = secure_filename(file.filename.encode('utf-8'))
               file_path = './uploads/'+file.filename.encode('utf-8') #+"."+filename.resplit('.')[1]           
               file.save(file_path)
           print("update2")
...
def me_write():
    ...
    if request.method == "GET":
        ...
        save_title=request.form.get('notitle').encode('utf-8')
        save_content=request.form.get('nocontent').encode('utf-8')
        try:
            file = request.files['_file']
            pass
        except Exception as e:
        ...
        save_noticedb(idid=writerid, title=save_title.encode('utf-8'),content=save_content.encode('utf-8'), day=day_date(), files=file_path)
        return redirect(url_for('me_list'))
    return ''
